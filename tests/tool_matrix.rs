...
fn verifies_fast_coding_profile() {
  let names = catalog::tool_catalog_for_profile("fast-coding").into_iter().map(|tool| tool["name"].as_str().unwrap().to_string()).collect::<Vec<_>>();
...
fn run_file_tools(checked: &mut Vec<String>, root: &Path) {
  ...
  call_checked(checked, "dir-create", json!({ "paths": [path_text(&nested_dir)] }));
  call_checked(
    checked,
    "file-write",
    json!({
  ...
  call_checked(checked, "file-read", json!({ "paths": [path_text(&sample)] }));
  ...
  call_checked(checked, "path-stat", json!({ "paths": [path_text(&moved_path)] }));
  call_checked(
    checked,
    "file-write",
    json!({
...
fn run_search_tools(checked: &mut Vec<String>, root: &Path) {
  ...
  let session_id = first_batch_struct(&files)["sessionId"].as_str().unwrap().to_string();
  call_checked(
    checked,
    "search-get",
    json!({
  ...
  call_checked(checked, "search-stop", json!({ "sessionIds": [session_id] }));
}
...
fn run_inspect_tools(checked: &mut Vec<String>, root: &Path) {
  ...
  fs::write(config.join("service.json"), "{\"renewalThreshold\":0.82,\"name\":\"renewal\"}\n").unwrap();
  ...
  assert_eq!(data["answers"][2]["value"]["values"]["/renewalThreshold"], 0.82);
...
fn run_git_tools(checked: &mut Vec<String>, root: &Path) {
  ...
  assert_ne!(tool_struct(&rewritten)["oid"].as_str().unwrap(), amended_oid);
  ...
  let zero_ctx = call_checked(checked, "git-diff", json!({ "path": path_text(&repo), "contextLines": 0 }));
  let wide_ctx = call_checked(checked, "git-diff", json!({ "path": path_text(&repo), "contextLines": 5 }));
  ...
  fs::write(&ctx, seed.replace("row 10\n", "row 10 staged\n")).unwrap();
  let staged = call_checked(checked, "git-add", json!({ "path": path_text(&repo), "update": true }));
...
fn call_checked(checked: &mut Vec<String>, name: &str, args: Value) -> Value {
  ...
  assert_ne!(response.get("isError").and_then(Value::as_bool), Some(true), "{name} failed: {response:#}");
...
fn batch_text(response: &Value) -> &str {
  response["structuredContent"]["data"]["content"][0]["text"].as_str().unwrap_or_default()
}

fn catalog_names() -> Vec<String> {
  let response = server::handle_line(r#"{"jsonrpc":"2.0","id":1,"method":"tools/list"}"#).unwrap();
  let mut names = response["result"]["tools"].as_array().unwrap().iter().map(|tool| tool["name"].as_str().unwrap().to_string()).collect::<Vec<_>>();
  names.sort();
  names
}
...
fn temp_root(label: &str) -> PathBuf {
  ...
  std::env::current_dir().unwrap().join("target").join(format!("rust-fs-mcp-{label}-{stamp}"))
}
