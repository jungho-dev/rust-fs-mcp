use rust_fs_mcp::core::response::{RawResult, normalize_tool_result};
use serde_json::json;
...
fn main() {
    ...
    for _ in 0..3 {
        bench("normalize_only_15k", 50_000, || {
            let raw = RawResult::structured(
                body15.clone(),
                json!({ "path": "p", "bytes": 15000, "lineCount": 1 }),
            );
    ...
    for _ in 0..3 {
        bench("normalize_only_95k", 50_000, || {
            let raw = RawResult::structured(
                body95.clone(),
                json!({ "path": "p", "bytes": 95000, "lineCount": 1 }),
            );
            let _ = normalize_tool_result("file-read", raw, Duration::from_millis(0));
        });
    }
