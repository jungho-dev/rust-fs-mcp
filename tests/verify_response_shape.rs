...
fn main() {
  ...
  let text = normalized["structuredContent"]["data"]["text"].as_str().unwrap_or("");
  let preview: String = text.chars().take(300).collect();
  println!("{preview}");

  let contains_reading = display.contains("Reading") || text.contains("Reading ") && text.contains(" chars");
